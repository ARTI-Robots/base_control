...
AbstractMotor::AbstractMotor() : predication_time_out_(0.11)
{
  ...
  SPDLOG_ROS_DEBUG_STREAM("VescMotor::VescMotor::7");

  // Measurement noise covariance matrix (R):
  cv::setIdentity(state_estimation_filter_.measurementNoiseCov, 1e-1);
  SPDLOG_ROS_DEBUG_STREAM("VescMotor::VescMotor::8");

  // Priori error estimate covariance matrix (P'(k)): P'(k)=A*P(k-1)*At + Q):
  cv::setIdentity(state_estimation_filter_.errorCovPre);
  SPDLOG_ROS_DEBUG_STREAM("VescMotor::VescMotor::9");
}
...
void AbstractMotor::correct(double estimate, bool is_mockup)
{
  ...
  SPDLOG_ROS_DEBUG_STREAM("VescMotor::correct: estimate: " << estimate);

  ros::Time now = ros::Time::now();
  last_correction_time_ = now;
  if (predict(now)) // only correct if prediction can be performed
  {
    SPDLOG_ROS_DEBUG_STREAM("VescMotor::correct::3");

    correct(estimate);
  }
  else if (!is_mockup)
  {
    SPDLOG_ROS_WARN("Skipping state correction due to failed prediction");
  }

  SPDLOG_ROS_DEBUG_STREAM("VescMotor::correct: corrected estimate: "
...
{
  SPDLOG_ROS_DEBUG_STREAM("VescMotor::predict::1");

  if (time > last_prediction_time_)
  {
    SPDLOG_ROS_DEBUG_STREAM("VescMotor::predict::2");

    if (!last_prediction_time_.isZero())
    {
      SPDLOG_ROS_DEBUG_STREAM("VescMotor::predict::3");

      if (!last_correction_time_.isZero()
          && ((time > last_correction_time_) && ((time - last_correction_time_) > predication_time_out_)))
      {
        SPDLOG_ROS_WARN_STREAM("no state update received in " << (time - last_correction_time_).toSec()
    ...
    SPDLOG_ROS_DEBUG_STREAM("VescMotor::predict::4");

    last_prediction_time_ = time;
    return true;
  }

  SPDLOG_ROS_DEBUG_STREAM("VescMotor::predict::5");
