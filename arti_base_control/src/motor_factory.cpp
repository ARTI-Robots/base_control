...
{
  DriveMotorPtr motor = std::make_shared<VescDriveMotor>(private_nh, driver_factory_, control_interval_);
  if (publish_motor_states_)
  {
    motor = std::make_shared<PublishingDriveMotor>(private_nh, motor);
  }
...
{
  SteeringMotorPtr motor = std::make_shared<VescSteeringMotor>(private_nh, driver_factory_, control_interval_);
  if (publish_motor_states_)
  {
    motor = std::make_shared<PublishingSteeringMotor>(private_nh, motor);
  }
